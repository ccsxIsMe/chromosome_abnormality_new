...
def default_save_dir(config_path, ckpt_path):
    ...
    return str(project_root / "outputs" / "posthoc_eval" / f"{ckpt_parent}__{config_stem}__chr_gmm_bank")
...
def build_chr_gmm_bank(
    ...
    for chromosome_id, bank_rows in grouped.items():
        ...
        actual_num_components = min(int(num_components), int(emb.shape[0]))
        if actual_num_components <= 0:
            raise ValueError("--num_components must be > 0")

        gmm = GaussianMixture(
            n_components=actual_num_components,
        ...
        cluster_assignments = gmm.predict(emb)
        cluster_counts = np.bincount(cluster_assignments, minlength=actual_num_components).astype(np.int64)
        train_log_probs = gmm.score_samples(emb)

        gmm_bank[chromosome_id] = {
            "gmm": gmm,
            "num_train_samples": int(emb.shape[0]),
            "num_components": int(actual_num_components),
            "cluster_counts": cluster_counts,
            "train_log_prob_mean": float(train_log_probs.mean()),
            "train_log_prob_std": float(train_log_probs.std()),
        }

    if not gmm_bank:
        raise ValueError("GMM bank is empty. Expected normal training samples.")
    return gmm_bank


def score_rows_with_gmm_bank(rows, gmm_bank, normalize_embeddings=True):
    scored_rows = []
    all_entries = list(gmm_bank.values())

    for row in rows:
        chromosome_id = str(row["chromosome_id"])
        bank_entry = gmm_bank.get(chromosome_id)
        if bank_entry is None:
            log_probs = []
            for fallback_entry in all_entries:
                gmm = fallback_entry["gmm"]
                query = np.asarray(row["embedding"], dtype=np.float32)
                if normalize_embeddings:
                    denom = max(float(np.linalg.norm(query)), 1e-12)
                    query = query / denom
                log_probs.append(float(gmm.score_samples(query[None, :])[0]))
            log_prob = float(max(log_probs))
        else:
            query = np.asarray(row["embedding"], dtype=np.float32)
            if normalize_embeddings:
                denom = max(float(np.linalg.norm(query)), 1e-12)
                query = query / denom
            log_prob = float(bank_entry["gmm"].score_samples(query[None, :])[0])

        scored = dict(row)
        scored["gmm_log_prob"] = log_prob
        scored["gmm_anomaly_score"] = float(-log_prob)
...
def write_summary_table(output_path, experiment_tag, global_eval, chr_eval, num_components, covariance_type, reg_covar):
    lines = [
        f"# Embedding Chromosome-Conditioned GMM Bank Summary ({experiment_tag})",
        "",
        f"- num_components_per_chr: `{num_components}`",
...
def main():
    ...
    parser.add_argument("--covariance_type", default="diag", choices=["full", "tied", "diag", "spherical"])
    ...
    train_rows = score_rows_with_gmm_bank(
        train_rows,
        gmm_bank,
        normalize_embeddings=args.normalize_embeddings,
    )
    val_rows = score_rows_with_gmm_bank(
        val_rows,
        gmm_bank,
        normalize_embeddings=args.normalize_embeddings,
    )
    test_rows = score_rows_with_gmm_bank(
        test_rows,
        gmm_bank,
        normalize_embeddings=args.normalize_embeddings,
    )

    score_col = "gmm_anomaly_score"
    ...
    results = {
        ...
        "gmm_bank_details": {
            chromosome_id: {
                ...
                "num_components": int(bank_entry["num_components"]),
                "cluster_counts": [int(v) for v in bank_entry["cluster_counts"].tolist()],
                "train_log_prob_mean": float(bank_entry["train_log_prob_mean"]),
                "train_log_prob_std": float(bank_entry["train_log_prob_std"]),
            }
            for chromosome_id, bank_entry in gmm_bank.items()
        },
        "embedding_dim": int(next(iter(train_rows))["embedding"].shape[0]),
        "global_eval": {
    ...
    print(f"Saved GMM bank evaluation to {results_path}")
