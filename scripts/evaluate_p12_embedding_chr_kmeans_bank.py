...
def default_save_dir(config_path, ckpt_path):
    ...
    return str(project_root / "outputs" / "posthoc_eval" / f"{ckpt_parent}__{config_stem}__chr_kmeans_bank")
...
def build_chr_prototype_bank(rows, num_prototypes, normalize_embeddings=True, max_per_chr=None, seed=42):
    ...
    for chromosome_id, bank_rows in grouped.items():
        ...
        actual_num_prototypes = min(int(num_prototypes), int(emb.shape[0]))
        if actual_num_prototypes <= 0:
            raise ValueError("num_prototypes must be > 0")

        if emb.shape[0] == actual_num_prototypes:
            prototypes = emb.copy()
            cluster_counts = np.ones((actual_num_prototypes,), dtype=np.int64)
            bank_type = "identity"
        else:
            kmeans = KMeans(
                n_clusters=actual_num_prototypes,
                random_state=seed,
                n_init=10,
            )
            assignments = kmeans.fit_predict(emb)
            prototypes = kmeans.cluster_centers_.astype(np.float32)
            prototypes = maybe_normalize_embeddings(prototypes, normalize_embeddings)
            cluster_counts = np.bincount(assignments, minlength=actual_num_prototypes).astype(np.int64)
            bank_type = "kmeans"

        prototype_bank[chromosome_id] = {
            "prototypes": prototypes,
            "cluster_counts": cluster_counts,
            "num_train_samples": int(emb.shape[0]),
            "bank_type": bank_type,
        }

    if not prototype_bank:
        raise ValueError("Prototype bank is empty. Expected normal training samples.")
...
def score_rows_with_prototype_bank(
    rows,
    prototype_bank,
    ...
    scored_rows = []
    all_bank_prototypes = np.concatenate(
        [bank_entry["prototypes"] for bank_entry in prototype_bank.values()],
        axis=0,
    )

    for row in rows:
        chromosome_id = str(row["chromosome_id"])
        bank_entry = prototype_bank.get(chromosome_id)
        if bank_entry is None:
            bank_embeddings = all_bank_prototypes
        else:
            bank_embeddings = bank_entry["prototypes"]
        ...
        prototype_score = compute_knn_distance(
            query_embedding=query,
            bank_embeddings=bank_embeddings,
            distance=distance,
            knn_k=topk_prototypes,
        )

        scored = dict(row)
        scored["prototype_bank_score"] = float(prototype_score)
...
def build_global_quantile_thresholds(train_df, quantiles, score_col):
    normal_scores = train_df.loc[train_df["label"].astype(int) == 0, score_col].astype(float).to_numpy()
...
def write_summary_table(output_path, experiment_tag, global_eval, chr_eval, distance, num_prototypes, topk_prototypes):
    lines = [
        ...
        f"- topk_prototypes_for_score: `{topk_prototypes}`",
        ...
        "|---|---:|---:|---:|---:|",
        (
            "| Prototype bank global val-best | "
        ...
        (
            "| Prototype bank chr-conditioned | "
...
def main():
    ...
    parser.add_argument("--topk_prototypes", type=int, default=1)
    ...
    if int(args.num_prototypes) <= 0:
        raise ValueError("--num_prototypes must be > 0")
    if int(args.topk_prototypes) <= 0:
        raise ValueError("--topk_prototypes must be > 0")
    ...
    seed = cfg.get("seed", 42) if args.seed is None else int(args.seed)
    ...
    prototype_bank = build_chr_prototype_bank(
        train_rows,
    ...
    train_rows = score_rows_with_prototype_bank(
        train_rows,
        prototype_bank,
        normalize_embeddings=args.normalize_embeddings,
    ...
    val_rows = score_rows_with_prototype_bank(
        val_rows,
        prototype_bank,
        normalize_embeddings=args.normalize_embeddings,
        distance=args.distance,
    ...
    test_rows = score_rows_with_prototype_bank(
        test_rows,
        prototype_bank,
        normalize_embeddings=args.normalize_embeddings,
        distance=args.distance,
    ...
    score_col = "prototype_bank_score"
    train_df = records_to_dataframe(train_rows, score_column=score_col)
    val_df = records_to_dataframe(val_rows, score_column=score_col)
    test_df = records_to_dataframe(test_rows, score_column=score_col)

    best_threshold, best_score, best_stats = search_best_threshold(
        y_true=val_df["label"].astype(int).tolist(),
        y_score=val_df[score_col].astype(float).tolist(),
        metric="f1",
        higher_score_more_positive=True,
    )
    test_metrics_val_best = compute_score_based_metrics(
        y_true=test_df["label"].astype(int).tolist(),
        y_score=test_df[score_col].astype(float).tolist(),
        threshold=best_threshold,
        higher_score_more_positive=True,
    )

    global_quantile_thresholds = build_global_quantile_thresholds(train_df, quantiles, score_col)
    ...
    chr_quantile_sweep = []
    for quantile in quantiles:
        chr_thresholds = compute_chr_quantile_thresholds(train_df, score_col, quantile)
    ...
    test_df["pred_label_global_valbest"] = (test_df[score_col].astype(float) >= float(best_threshold)).astype(int)
    ...
    results = {
        ...
        "topk_prototypes": int(args.topk_prototypes),
        ...
        "prototype_bank_sizes": {
            chromosome_id: int(bank_entry["prototypes"].shape[0])
            for chromosome_id, bank_entry in prototype_bank.items()
        },
        "prototype_bank_details": {
            chromosome_id: {
                "num_train_samples": int(bank_entry["num_train_samples"]),
                "num_prototypes": int(bank_entry["prototypes"].shape[0]),
                "cluster_counts": [int(v) for v in bank_entry["cluster_counts"].tolist()],
                "bank_type": str(bank_entry["bank_type"]),
            }
            for chromosome_id, bank_entry in prototype_bank.items()
        },
        "embedding_dim": int(next(iter(prototype_bank.values()))["prototypes"].shape[1]),
        "global_eval": {
    ...
    write_summary_table(
        ...
        args.distance,
        args.num_prototypes,
        args.topk_prototypes,
    )

    print(f"Saved KMeans prototype-bank evaluation to {results_path}")
