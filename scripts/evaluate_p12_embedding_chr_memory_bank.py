...
def write_summary_table(output_path, experiment_tag, global_eval, chr_eval, distance, knn_k):
    lines = [
        f"# Embedding Chromosome-Conditioned Memory Bank Summary ({experiment_tag})",
        "",
        f"- distance: `{distance}`",
        f"- knn_k: `{knn_k}`",
...
def main():
    ...
    results = {
        "method": "embedding_chr_memory_bank",
