...
N_POINTS = 7
MODE_NAME = "manual_7point_web"

POINT_NAMES = ['upper_left', 'upper_right', 'upper_midline', 'centromere', 'lower_midline', 'lower_left', 'lower_right']
POINT_LABELS = ['1 上左肩点 / Upper-left shoulder', '2 上右肩点 / Upper-right shoulder', '3 上半段中心辅助点 / Upper midline helper', '4 着丝粒 / Centromere', '5 下半段中心辅助点 / Lower midline helper', '6 下左肩点 / Lower-left shoulder', '7 下右肩点 / Lower-right shoulder']
...
def build_manual_path_from_points(points_yx: np.ndarray, mask: np.ndarray, n_cap: int = 22) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    """
    7-point mode. points_yx order:
      0 upper_left
      1 upper_right
      2 upper_midline helper
      3 centromere
      4 lower_midline helper
      5 lower_left
      6 lower_right

    Path:
      top_tip -> top_mid -> upper_midline -> centromere -> lower_midline -> bottom_mid -> bottom_tip

    This mode is better for obviously curved/S-shaped chromosomes.
    """
    pts = np.asarray(points_yx, dtype=np.float64)
    if pts.shape != (7, 2):
        raise ValueError("Need exactly 7 points in y,x order")

    p_ul, p_ur, p_um, p_c, p_lm, p_ll, p_lr = pts

    p_top = 0.5 * (p_ul + p_ur)
    p_bottom = 0.5 * (p_ll + p_lr)

    t_top_in = _choose_normal_from_pair(p_ul, p_ur, p_um - p_top)
    t_bottom_out = _choose_normal_from_pair(p_ll, p_lr, p_bottom - p_lm)

    p_top_tip = _march_to_mask_boundary(p_top, -t_top_in, mask)
    p_bottom_tip = _march_to_mask_boundary(p_bottom, t_bottom_out, mask)

    # Use a polyline with explicit helper points; fit_and_resample_centerline()
    # will fit a smooth B-spline to it later.
...
""".replace("__N_POINTS__", str(N_POINTS)).replace("__LABELS__", json.dumps(['上左肩点', '上右肩点', '上半段中心辅助点', '着丝粒', '下半段中心辅助点', '下左肩点', '下右肩点'], ensure_ascii=False)).replace("__INSTRUCTION__", "点击顺序：1 上左肩点 → 2 上右肩点 → 3 上半段中心辅助点 → 4 着丝粒 → 5 下半段中心辅助点 → 6 下左肩点 → 7 下右肩点。点满 7 个后右侧自动预览。")
...
def main():
    ap = argparse.ArgumentParser(description="Web-based interactive chromosome straightening with 7 manual points.")
